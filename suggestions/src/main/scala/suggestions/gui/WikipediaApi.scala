...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def recovered: Observable[Try[T]] = { 
      obs.materialize.map(n => {
        n match {
          case Notification.OnNext(value) => Success(value)
          //case Notification.OnCompleted() => Success(value)
          case Notification.OnError(error) => Failure(error)
        }
      })
    }
